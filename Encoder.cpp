...
#else
std::wstring Encoder::readFile(const char *filename) {
  return std::wstring(L"https://www.youtube.com/watch?v=OloXRhesab0");
}
#endif
...
void Encoder::Encode(std::vector<bool> &dataFinal) {
  ...
  case ALPHANUM:
    ...
    for (auto &c : read)
      rawData.push_back(c2alpha(c));
    break;
  case NUMERIC:
    std::cout << "Mode: Numeric encoding\n";
    for (auto &c : read)
      rawData.push_back(c - 48);
    break;
  case KANJI:
  ...
  int i = 0;
  while (i < 40 &&
         versionSize[4 * i + info->error_level][info->encoding] < dataSize)
    i++;
  if (i >= 40) {
    ...
    switch (info->encoding) {
    case BYTE_ENC:
      std::cout << "Byte Mode: 2953";
      break;
    case ALPHANUM:
      std::cout << "Alphanumeric: 4296";
      break;
    case NUMERIC:
      std::cout << "Numeric: 7089";
      break;
    case KANJI:
      std::cout << "Kanji: 1817";
      break;
  ...
  if (info->encoding == NUMERIC) {
    ...
    for (auto it = rawData.begin(); it != rawData.end(); it++) {
      if (*it >= 0x8140 && *it <= 0x9FFC) {
        *it -= 0x8140;
        encoded.push_back(
            varNum(((*it >> 8) * 0xC0) * ((byte)0xFFFF & *it), 13));
  ...
  switch (info->encoding) {
  case BYTE_ENC:
    pushBits(0x4, dataFinal, 4);
    break;
  case ALPHANUM:
    pushBits(0x2, dataFinal, 4);
    break;
  case NUMERIC:
    pushBits(0x1, dataFinal, 4);
    break;
  case KANJI:
    pushBits(0x8, dataFinal, 4);
    break;
  }
  ...
  int lengthRequired =
      errorBlockInfo[info->version * 4 + info->error_level][0] * 8;
