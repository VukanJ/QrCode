...
void MaskInfo::applyMask(QR_Field& qr_field,int m)
{
	...
	switch (m){
	case 0: mask_alg = [](int i, int j)->bool {return (i + j) % 2 == 0; }; break;
	case 1: mask_alg = [](int i, int j)->bool {return i % 2 == 0; }; break;
	case 2: mask_alg = [](int i, int j)->bool {return j % 3 == 0; }; break;
	case 3: mask_alg = [](int i, int j)->bool {return (i + j) % 3 == 0; }; break;
	case 4: mask_alg = [](int i, int j)->bool {return (int)(floor(i / 2) + floor(j / 3)) % 2 == 0; }; break;
	case 5: mask_alg = [](int i, int j)->bool {return (i*j) % 2 + (i*j) % 3 == 0; }; break;
	case 6: mask_alg = [](int i, int j)->bool {return ((i*j) % 2 + (i*j) % 3) % 2 == 0; }; break;
	case 7: mask_alg = [](int i, int j)->bool {return ((i + j) % 2 + (i*j) % 3) % 2 == 0; }; break;
	default:std::cerr << "INVALID MASKING PATTERN SELECTED\n"; exit(1); break;
	}
	for (int i = 0; i < info->size; i++)
		for (int j = 0; j < info->size; j++)
