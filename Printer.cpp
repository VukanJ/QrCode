...
void Printer::printAllPatterns(QR_Field &qr_field) {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    for (byte p = 0x1; c != 7; p <<= 1, c++) {
      qr_field[i][c].write_lock(finder_dat[i] & p ? true : false,
                                ModType::M_FINDER);
      qr_field[i][c + info->size - 7].write_lock(
          finder_dat[i] & p ? true : false, ModType::M_FINDER);
      qr_field[i + info->size - 7][c].write_lock(
          finder_dat[i] & p ? true : false, ModType::M_FINDER);
    }
  }
  c = 0;
        ...
        if (qr_field[x][y].type == M_VOID)
          for (auto &p : align_dat) {
            ...
            for (byte b = (byte)0x1; c != 5; b <<= 1, c++) {
              qr_field[x - 2 + c][y - 2 + d].write_lock(p & b ? true : false,
                                                        ModType::M_ALIGN);
            }
            d++;
          }
...
void Printer::printFinal(sf::Image &qrCode, QR_Field &qr_field) {
  ...
  for (int i = 0; i < info->size; i++)
    for (int j = 0; j < info->size; j++) {
      qrCode.setPixel(i + 4, j + 4, qr_field[i][j].state() ? sf::Color::Black
                                                           : sf::Color::White);
    }
}
...
void Printer::printDebugFinal(sf::Image &qrCode, QR_Field &qr_field) {
  ...
  for (int i = 0; i < info->size; i++) {
    for (int j = 0; j < info->size; j++) {
      ...
      case M_SEP:
      case M_FINDER:
        tempCol = tempCol.r == 0 ? sf::Color(0, 50, 0) : sf::Color(0, 200, 0);
        break;
      case M_TIMING:
        tempCol =
            tempCol.r == 0 ? sf::Color(0, 0, 100) : sf::Color(130, 130, 200);
        break;
      case M_ALIGN:
        tempCol = tempCol.r == 0 ? sf::Color(30, 0, 0) : sf::Color(200, 0, 0);
        break;
      case M_DATA:
        tempCol =
            tempCol.r == 0 ? sf::Color(100, 0, 0) : sf::Color(200, 100, 100);
        break;
      case M_FORMAT:
        tempCol = tempCol.r == 0 ? sf::Color::Black : sf::Color::White;
        break;
      case M_DARK_MODULE:
        tempCol = sf::Color::Blue;
        break;
      default:
        tempCol = sf::Color(50, 50, 50);
        break;
      }
